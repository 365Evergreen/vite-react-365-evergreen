...
const GET_IN_TOUCH_FORM_URL = new URL('./00001.json', import.meta.url).toString()
...
function normalizeGetInTouchForm(payload: unknown): GetInTouchFormDefinition {
  if (typeof payload !== 'object' || payload === null) {
    throw new Error('Get in touch form definition is invalid')
  }

  const raw = payload as Record<string, unknown>
  const id = typeof raw.id === 'string' ? raw.id : 'get-in-touch-form'
  const title = typeof raw.title === 'string' ? raw.title : 'Get in touch form'
...
async function loadGetInTouchFormDefinition(): Promise<GetInTouchFormDefinition> {
  if (!getInTouchFormPromise) {
    getInTouchFormPromise = fetch(GET_IN_TOUCH_FORM_URL, { headers: { Accept: 'application/json' } })
      .then((response) => {
        if (!response.ok) {
          throw new Error(`Failed to load get in touch form (HTTP ${response.status})`)
        }
